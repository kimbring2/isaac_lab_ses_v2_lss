...
class Se3Composite(DeviceBase):
    ...
    * gripper: a binary command to open or close the gripper.
    ...
    """
    def __init__(self, cfg: Se3CompositeCfg):
        ...
        self._keyboard_sub = self._input.subscribe_to_keyboard_events(
            self._keyboard, lambda event, *args, obj=weakref.proxy(self): obj._on_keyboard_event(event, *args),
        )
        ...
        self._disable_translation = True
    ...
    def _find_device(self):
        ...
        for _ in range(5):
            for device in hid.enumerate():
                #print("device[\"product_string\"]: ", device["product_string"])

                if (device["product_string"] == "SpaceMouse Compact" or device["product_string"] == "SpaceMouse Wireless"
                ...
                if self._device_name == "3Dconnexion Universal Receiver":
                    if data[0] == 1:
                        if self._disable_translation == False:
                            self._delta_pos[0] = self.pos_sensitivity * convert_buffer(data[1], data[2]) * 1.0
                            self._delta_pos[2] = self.pos_sensitivity * convert_buffer(data[3], data[4]) * 1.0
                            self._delta_pos[1] = self.pos_sensitivity * convert_buffer(data[5], data[6])
                        ...
                        if self._disable_rotation == False:
                            ...
                            self._delta_rot[1] = self.rot_sensitivity * convert_buffer(data[5 + 6], data[6 + 6])
                else:
                    if data[0] == 1:
    ...
    def _on_keyboard_event(self, event, *args, **kwargs):
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            #print(f"event.type: {event.type}, event.input.name: {event.input.name}")
    ...
    def _create_key_bindings(self):
        ...
        self._INPUT_KEY_MAPPING = {
            ...
            # x-axis (forward)
            "W": np.asarray([1.0, 0.0, 0.0]) * self.pos_sensitivity, "S": np.asarray([-1.0, 0.0, 0.0]) * self.pos_sensitivity,
            
            # y-axis (left-right)
            "A": np.asarray([0.0, 1.0, 0.0]) * self.pos_sensitivity, "D": np.asarray([0.0, -1.0, 0.0]) * self.pos_sensitivity,
            ...
            # roll (around x-axis)
            "Z": np.asarray([1.0, 0.0, 0.0]) * self.rot_sensitivity, "X": np.asarray([-1.0, 0.0, 0.0]) * self.rot_sensitivity,
            
            # pitch (around y-axis)
            "T": np.asarray([0.0, 1.0, 0.0]) * self.rot_sensitivity, "G": np.asarray([0.0, -1.0, 0.0]) * self.rot_sensitivity,
            
            # yaw (around z-axis)
            "C": np.asarray([0.0, 0.0, 1.0]) * self.rot_sensitivity, "V": np.asarray([0.0, 0.0, -1.0]) * self.rot_sensitivity,
        }


@dataclass
class Se3CompositeCfg(DeviceCfg):
    """Configuration for SE3 space mouse devices."""
    gripper_term: bool = True
    pos_sensitivity: float = 0.4
    rot_sensitivity: float = 0.8
    retargeters: None = None
    class_type: type[DeviceBase] = Se3Composite